...
class Map():
    def __init__(self, traffic_situation = "two_lane-intersection"):
        ...
        if traffic_situation == "two-lane_intersection" :
            self.map_dict =  {'0': {'type': 'crossing_dr', 'polygon': Polygon([(l+0.5*w, l), (l+w, l), (l+w, l+0.5*w), (l+0.5*w, l+0.5*w)]), 'color': 'w', 'transparency': 1},
                        '1': {'type': 'crossing_ur', 'polygon': Polygon([(l+0.5*w, l+0.5*w), (l+w, l+0.5*w), (l+w, l+w), (l+0.5*w, l+w)]), 'color': 'w', 'transparency': 1},
                        '2': {'type': 'crossing_ul', 'polygon': Polygon([(l, l+0.5*w), (l+0.5*w, l+0.5*w), (l+0.5*w, l+w), (l, l+w)]), 'color': 'w', 'transparency': 1},
                        '3': {'type': 'crossing_dl', 'polygon': Polygon([(l, l), (l+0.5*w, l), (l+0.5*w, l+0.5*w), (l, l+0.5*w)]), 'color': 'w', 'transparency': 1},
...
class Robot():
    ...
    def reset(self, start, task):
        ...
        if start == 'down':
            ...
            self.pos_uri = URIRef("http://example.com/intersection/road_down/lane1")
        elif start == 'up':
            self.pos = np.array([l+0.25*w,2*l+w-self.length/2])
            self.yaw = -0.5*math.pi
            self.pos_uri = URIRef("http://example.com/intersection/road_up/lane1")
        elif start == 'left':
            self.pos = np.array([self.length/2,l+0.25*w])
            self.yaw = 0
            self.pos_uri = URIRef("http://example.com/intersection/road_left/lane1")
        elif start == 'right':
...
class Obstacle():
    ...
    def plot(self):
        #self.polygon = trans(self.pos, self.yaw, self.length, self.width)
        plt.fill(*self.polygon.exterior.xy, edgecolor='red', hatch='//', fill=False)
